...
- [x] Adicionar casos a `tests/test_collaborators_aggregate.php` que
...
- [x] Todos os asserts existentes continuam a passar.

## 2. Schema: colunas novas em `time_entries`

- [x] Em `includes/database.php`, adicionar bloco idempotente após o
      ...
      ```
- [x] Verificar com `sqlite3 data/sonar.db ".schema time_entries"` que
      as colunas foram criadas.

## 3. Sync: capturar task name/url no upsert

- [x] `db_upsert_time_entries` passa a ler `$e['task']['name']` e a
      construir `task_url = 'https://app.clickup.com/t/' + task_id`.
      Gravar nas colunas novas. `parent_task_id` e `parent_name` ficam
      NULL nesta tarefa.
- [x] O INSERT OR REPLACE estende-se para cobrir os novos campos,
      mantendo retrocompatibilidade (se task_name não vier, escreve
      NULL).
- [x] Correr um sync manual (botão Sync) e confirmar via SQL que
...
- [x] Em `api/sync_time_entries.php`, após capturar os entries brutos e
...
- [x] `db_upsert_time_entries` aceita um 3º parâmetro opcional
      `$parentMap` e preenche `parent_task_id` e `parent_name` quando
      existem no mapa.
- [x] Progress UI: mostrar "A resolver posts... (X/Y)" durante esta
      fase, para o user saber que é este passo que demora.

## 5. Endpoint: expor post_name/post_url no agregado

- [x] `collab_aggregate_weeks` passa a agrupar por `post_id`:
...
- [x] Cada linha em `days_tasks[dow]` passa a ter
      `{post_id, post_name, post_url, duration_ms, hours}` (substitui
      `task_id`/`task_name`/`task_url`).
- [x] Rodar `tests/test_collaborators_aggregate.php` — todos os asserts
      novos e velhos passam.

## 6. Frontend: popup usa post_name/post_url

- [x] `renderCollabDayRow` em `assets/js/app.js`:
  - label = `t.post_name || 'Task ' + t.post_id` (fallback só se name
    vazio)
  - href = `t.post_url || 'https://app.clickup.com/t/' + t.post_id`
- [x] Remover o ramo "entry sem task" — não é atingível com sync
      actual.

## 7. Validação

- [x] Criar `validation.md` ao lado da spec com:
...
- [x] `git mv specs/10-active/F03-time-entries-post-context specs/20-done/`
- [x] Nenhum TODO pendente no código.
